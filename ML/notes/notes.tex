...
\geometry{left=0.8cm, right=0.8cm, top=1.5cm, bottom=1.5cm}
...
\author{\kaishu\zihao{-3} 酥雨 \qquad zusuyu@stu.pku.edu.cn}
...
\begin{itemize}
	...
	\item 定义 $\Phi_{\mathcal F} = \{\phi_f | f \in \mathcal F\}$. 由于以下不会出现超过一个 hypothesis space, 故省略下标简记为 $\Phi$.
\end{itemize}
...
这么做的意义是什么? 意义是可以先只考虑内层的 $\mathbb P_{\sigma}$ 而不管 $S \sim D^n$ 的选取. 看似强行取的随机排列 $\sigma$ 是为了内层可以被 bound, 不然 $\mathbbm 1\left[\left|\frac1n\sum\limits_{i=1}^{n}\phi(z_i) - \frac1n\sum\limits_{i=n+1}^{2n}\phi(z_i)\right| \ge \varepsilon \right]$ 还不太方便处理.
...
\begin{theorem}[Sion's Minimax Theorem]
	$\mathcal X$ 是一个紧的凸集, $\mathcal Y$ 是一个凸集, $f: \mathcal X \times \mathcal Y \to \mathbb R$ 是连续函数, 且对任意 $x \in \mathcal X$, $f(x, \cdot)$ 是 $\mathcal Y$ 上的凹函数, 对任意 $y \in \mathcal Y$, $f(\cdot, y)$ 是 $\mathcal X$ 上的凸函数, 则 $$\min_{x \in \mathcal X} \max_{y \in \mathcal Y}f(x, y) = \max_{y \in \mathcal Y} \min_{x \in \mathcal X}f(x, y)$$
	\label{Sion}
\end{theorem}
...
考虑一种如下形式的优化问题 \begin{align*}
	(P)\ \min\limits_{ x} & \quad f( x) \\
	\text{s.t.} & \quad g_i( x) \le 0 \ (i \in [m]) \\
	& \quad h_i( x) = 0 \ (i \in [n])
\end{align*}

其中 $f, g_i$ 是下凸函数, $h_i$ 是线性函数.

利用拉格朗日乘数法来解这个优化问题. 具体的, 定义拉格朗日乘子 $$L(x, \lambda, \mu) = f( x) + \sum_{i=1}^m\lambda_ig_i(x) + \sum_{j=1}^n\mu_jh_j(x)$$

一方面, 原问题等价于 $\min\limits_{x}\max\limits_{\lambda \ge 0, \mu}L(x, \lambda, \mu)$; 另一方面, 考虑 $L(x, \lambda, \mu)$ 对于 $x$ 是凸函数, 对于 $\lambda, \mu$ 是线性函数, 故也是凹函数, 从而根据 Sion's Minimax Theorem(\cref{Sion})可知 $$\min\limits_{x}\max\limits_{\lambda \ge 0, \mu}L(x, \lambda, \mu) = \max\limits_{\lambda \ge 0, \mu}\min\limits_{x}L(x, \lambda, \mu)$$

对于 $\min\limits_{x}L(x, \lambda, \mu)$, 可以利用 $\frac{\partial L}{\partial x} = 0$ 解得 $x = \varphi(\lambda, \mu)$, 从而得到了原问题的对偶问题 \begin{align*}
	(D) \ \max_{\lambda, \mu} & \quad L(\varphi(\lambda, \mu), \lambda, \mu) \\
	\text{s.t.} & \quad \lambda_i \ge 0 \ (i \in [m])
\end{align*}

\begin{definition}[KKT condition]
	称 $x^*, \lambda^*, \mu^*$ 满足 KKT condition, 如果它们满足 \begin{itemize}
		\item Stationarity: $\nabla_x L|_{x^*, \lambda^*, \mu^*} = 0$.
		\item Primal Feasibility: $g_i(x^*) \le 0 \ (i \in [m]), h_i(x^*) = 0 \ (i \in [n])$.
		\item Dual feasibility: $\lambda^*_i \ge 0 \ (i \in [m])$.
		\item Complementary Slackness: $\lambda^*_ig_i(x^*) = 0 \ (i \in [m])$.
	\end{itemize}
\end{definition}
\begin{theorem}
	$x^*, \lambda^*, \mu^*$ 是原问题 $P$ 与对偶问题 $D$ 的解, 当且仅当它们满足 KKT condition.
\end{theorem}

\subsection{Linear Seperation}

$\mathbb R^d$ 上线性分类器的 hypothesis space 可以写为 $$\mathcal F = \{f(\mathbf x) = \mathbf w^{\text T}\mathbf x + b | \mathbf w \in \mathbb R^d, b \in \mathbb R\}$$

对于一个数据集 $S = \{(\mathbf x_1, y_1), \cdots, (\mathbf x_n, y_n)\}\in (\mathbb R^d \times \{\pm 1\})^n$, 我们希望知道 $S$ 是不是线性可分的, 即是否存在一个 $f \in \mathcal F$ 使 $\sum\limits_{i=1}^n\mathbbm 1[f(x_i) \neq y_i] = 0$. 这可以用线性规划来解决: \begin{align*}
	\max_{\mathbf w, b, t} & \quad t \\
	\text{s.t. } & \quad y_i(\mathbf w^{\text T}\mathbf x_i + b) \ge t \ (i \in [n])
\end{align*}

我们进一步地希望找到一个 $f$ 来最大化 margin, 即实现 \begin{align*}
	\max_{\mathbf w, b, t} & \quad t \\
	\text{s.t. } & \quad y_i(\mathbf w^{\text T}\mathbf x_i + b) \ge t \ (i \in [n]) \\
	& \quad \|\mathbf w\|_2 = 1
\end{align*}

虽然这个规划问题不好解, 但是可以证明与如下规划问题等价. \begin{align*}
	\min_{\mathbf w, b} & \quad \frac12\|\mathbf w\|_2^2 \\
	\text{s.t.} & \quad y_i(\mathbf w^{\text T}\mathbf x_i + b) \ge 1 \ (i \in [n])
\end{align*}
...
上一个规划问题的拉格朗日乘子为 $$L(\w, b, \lambda) = \frac12 \|\w\|_2^2 + \sum_{i=1}^n \lambda_i(1 - y_i(\w^{\text T}\x_i + b))$$

根据 KKT condition, 其最优解 $\w^*, b^*, \lambda^*$ 应当满足 \begin{align*}
	\w^* &= \sum_{i=1}^n \lambda^*_i y_i \x_i \\	
	0 &= \lambda^*_i (y_i({\w^*}^{\text T}\x_i + b^*) - 1) \ (i \in [n])
\end{align*}

最优解中的 $w^*$ 是 $y_i\x_i$ 的线性组合, 具体的, 它是满足 $y_i({\w^*}^{\text T}\x_i + b^*) - 1 = 0$ 的 $y_i\x_i$ 的线性组合, 也就是说, 只有在 margin 上的数据点才会影响到 $w^*$, 而这些点(这些 $y_i\x_i$)也被称为\obj{支持向量(support vector)}.

\subsection{Soft Margin SVM}

实际上很多情况下数据集 $S$ 都不是线性可分的, 因此可以适当放宽条件. 把原问题改写成 \begin{align*}
	\min_{\mathbf w, b, \varepsilon} & \quad \frac12\|\mathbf w\|_2^2 + C\sum_{i=1}^n\varepsilon_i\\
	\text{s.t.} & \quad y_i(\mathbf w^{\text T}\mathbf x_i + b) \ge 1 - \varepsilon_i\ (i \in [n]) \\
	& \quad \varepsilon_i \ge 0 \ (i \in [n])
\end{align*}

其对偶形式为 \begin{align*}
	\min_\lambda & \quad \frac12 \sum_{i=1}^n\sum_{j=1}^n \lambda_i\lambda_j\x_i^{\text T}\x_j - \sum_{i=1}^n \lambda_i \\
	\text{s.t.} & \quad \sum_{i=1}^n\lambda_iy_i = 0 \\
	& \quad 0 \le \lambda_i \le C \ (i \in [n])
\end{align*}

也可以换一种角度考虑 Soft Margin SVM. 定义三种 loss function: \begin{itemize}
	\item 01 loss: $\ell_{01}(x) =\mathbbm 1[x \le 0]$.
	\item hinge loss: $\ell_{\text{hinge}}(x) = \max\{1 - x, 0\}$.
	\item exponential loss: $\ell_{\text{exp}}(x) = \e^{-x}$.
\end{itemize}

不难发现 $\ell_{01}(x) \le \ell_{\text{hinge}}(x) \le \ell_{\text{exp}}(x)$. SVM 做的事情是找一个 $f$ 使 $\sum\limits_{i=1}^n \ell_{\text{hinge}}(y_if(\x_i)) = 0$, 而 Soft Margin SVM 做的事情则是找一个 $f$ 最小化 $\sum\limits_{i=1}^n \ell_{\text{hinge}}(y_if(\x_i))$.
...
\begin{proposition}
	$\alpha_t = \arg\min\limits_{\alpha}Z_t = \arg\min\limits_{\alpha} \sum\limits_{i=1}^{n}D_t(i)\exp(-\alpha y_i h_t(x_i))$.
\end{proposition}
\begin{proof}
	Recall that $\varepsilon_t = \sum\limits_{i=1}^{n}D_t(i)\mathbb I[y_i \neq h_t(x_i)]$, by applying \obj{AM–GM inequality} we have
	$$Z_t = \sum\limits_{i=1}^{n}D_t(i)\exp(-\alpha y_i h_t(x_i)) = \varepsilon_t \exp(\alpha) + (1 - \varepsilon_t)\exp(-\alpha) \ge 2\sqrt{\varepsilon_t(1 - \varepsilon_t)}$$
	where the equality holds if and only if $$\varepsilon_t \exp(\alpha) = (1 - \varepsilon_t)\exp(-\alpha) \Leftrightarrow \alpha = \frac12 \ln\frac{1 - \varepsilon_t}{\varepsilon_t} = \frac12 \ln\frac{1 + \gamma_t}{1 - \gamma_t} = \alpha_t$$
	where $\gamma_t = 1 - 2\varepsilon_t$ in the assignment of $\alpha_t$. This suggests that $\alpha_t = \arg\min\limits_{\alpha} Z_t$ as desired.
\end{proof}

\begin{proposition}
	$\prod\limits_{t=1}^{T}Z_t = \frac1n \sum\limits_{i=1}^{n}\exp\left(-y_i\sum\limits_{t=1}^{T}\alpha_t h_t(x_i)\right) = \frac1n \sum\limits_{i=1}^{n}\exp\left(-y_if(x_i)\right)$.
\end{proposition}
\begin{proof}
	Notice that $D_{t+1}(i) = \frac{D_t(i)\exp(-\alpha_t y_i h_t(x_i))}{Z_t}$, by iteratively substitute the term of $D_t(i)$ in the expression of $Z_T$ ($Z_T = \sum_{i=1}^{n}D_T(i)\exp(-\alpha_T y_i h_T(x_i))$), we can eventually obtain the following equality. \begin{equation*}
		\begin{split}
...
\begin{proposition}
	$\sum\limits_{i=1}^{n}D_{t+1}(i)\mathbb I[y_i \neq h_t(x_i)] = \frac12$.
\end{proposition}
...
\begin{proposition}
	$\mathbb P_{(x_i, y_i) \sim S} \left[F(x_i) \neq y_i\right] \le (1 - \gamma^2)^{T/2}$.
\end{proposition}
\begin{proof}
	\begin{align*}
		\begin{split}
...
VC Theorem(\cref{VC-theorem})给出了频率学派对 generalization gap 一致收敛的限制, 而在贝叶斯学派中, 具有相同地位的是如下的 PAC-Bayesian Theorem.
...
\begin{theorem}[PAC-Bayesian Theorem]
	...
	其中 $err_{X}(f)$ 表示 classifier $f$ 在数据集 $X$ 上的错误率, 即 $\mathbb P_{(x, y) \in X}[y \neq f(x)]$, $D_{KL}(\mathcal Q \| \mathcal P) = \mathbb E_{h \sim \mathcal Q}\left[\ln \frac{\mathcal Q_h}{\mathcal P_h}\right]$ 为概率分布 $\mathcal Q$ 与 $\mathcal P$ 的 KL 散度(相对熵).
	\label{PAC-Bayesian}
\end{theorem}
...
\begin{equation*}
	...
	\end{split}
\end{equation*}
其中第一行不等号使用了 Cauchy Inequality, 第三行使用了\cref{PAC-Bayesian-lem-1} 代入 $f(h) = n\Delta^2$, 第四行使用了\cref{PAC-Bayesian-lem-2}, with probability at least $1 - \delta$.
...
现在我们考虑 SVM 处理的线性分类问题. 假设 hypothesis space 为 $\mathcal F = \{f(\mathbf x) = \mathbf w^{\text T}\mathbf x | \mathbf w \in \mathbb R^d\}$, 考虑两个 distribution of classifiers $\mathcal P = \mathcal N(\mathbf 0, I_d), \mathcal Q = \mathcal N(\mu\mathbf w, I_d)$, 其中 $\|\mathbf w\|_2 = 1$, $\mu$ 是缩放系数. 此时 $g_{\mathcal Q}$ 就是传统理解下的 linear classifier $\mathbf w$ (这里不考虑常数 $b$).
