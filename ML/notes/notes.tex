...
\begin{proof}
	...
	考虑 \begin{align*}
		...
		\end{split}
	\end{align*}
	从而得到 $$\frac{\P{\mathcal A(D) = \hat{D}}}{\P{\mathcal A(D') = \hat{D}}} \le \exp\left(\frac{2\Delta u}{\sigma}\right) \le \exp\left(\frac{2}{n\sigma}\right) = \e^{\varepsilon}$$

	至于 $(\alpha, \beta)$-精确, 事实上在 $\mathcal A$ 的返回值不是实数后我们还没有定义 $(\alpha, \beta)$-精确到底是什么, 所以在这里重新定义一下: 称 $\mathcal A$ 满足 $(\alpha, \beta)$-精确, 如果 $$\P{u(D, \mathcal A(D)) \le \max_{\hat{D}}u(D, \hat{D}) - \alpha} \le \beta$$
